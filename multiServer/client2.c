...
void reqFn(){
	while(1){
		...
		strcpy(reqbuf,"resfifo2|");strcpy(sno,"");
...
void serviceFn(){
	while(1){
		...
		if(read(resfifo2,&resbuf,sizeof(char)*100)>0){
...
int main(){
	...
	mkfifo("resfifo2",S_IRUSR|S_IWUSR);
	resfifo2=open("resfifo2",O_RDONLY|O_NONBLOCK);
